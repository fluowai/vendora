...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'WhiteLabel_planId_fkey') THEN
    ALTER TABLE "WhiteLabel" ADD CONSTRAINT "WhiteLabel_planId_fkey"
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'WhiteLabel_ownerUserId_fkey') THEN
    ALTER TABLE "WhiteLabel" ADD CONSTRAINT "WhiteLabel_ownerUserId_fkey"
      FOREIGN KEY ("ownerUserId") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Tenant_whiteLabelId_fkey') THEN
    ALTER TABLE "Tenant" ADD CONSTRAINT "Tenant_whiteLabelId_fkey"
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'User_whiteLabelId_fkey') THEN
    ALTER TABLE "User" ADD CONSTRAINT "User_whiteLabelId_fkey"
      FOREIGN KEY ("whiteLabelId") REFERENCES "WhiteLabel"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END $$;
`;
...
try {
  ...
  console.log("Whitelabel schema is aligned.");
} finally {
  await pool.end();
}
